...
import user_pf from '../../assets/user_profile.jpg'
...
const PlayVideo = () => {
    ...
    const fetchOtherData = async () => {
        ...
        const comment = `https://youtube.googleapis.com/youtube/v3/commentThreads?part=snippet%2Creplies&videoId=${videoId}&key=${API_KEY}`;
        await fetch(comment).then(res => res.json()).then(data => setCmmtData(data.items));
    ...
    return (
        <div className='play-vdo'>
            ...
            <div className="play-vdo-info">
                <p>{apiData ? value_converter(apiData.statistics.viewCount) : "22K"} Views &bull;{apiData ? moment(apiData.snippet.publishedAt).fromNow() : ""}</p>
                <div>
            ...
            <div className="publisher">
                ...
                <div>
                    <p>{apiData ? apiData.snippet.channelTitle : "Little Star"}</p>
            ...
            {cmmtData.map((item, index) => {
                return (
                    <div key={index} className="comment">
                        <img src={item.snippet.topLevelComment.snippet.authorProfileImageUrl} alt="" />
                        <div>
                            <h4>{item.snippet.topLevelComment.snippet.authorDisplayName} <span>1 week ago</span></h4>
